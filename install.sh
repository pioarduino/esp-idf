...
"${ESP_PYTHON}" "${IDF_PATH}/tools/idf_tools.py" --non-interactive install install --targets="${TARGETS}"

FEATURES=$("${ESP_PYTHON}" "${IDF_PATH}/tools/install_util.py" extract features "$@")

echo "Installing Python environment and packages"
"${ESP_PYTHON}" "${IDF_PATH}/tools/idf_tools.py" --non-interactive install install-python-env --features="${FEATURES}"
